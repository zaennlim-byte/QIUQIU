...
export interface StudyChapter {
    ...
    content?: string;
    // 分段阅读相关
    paragraphs?: string[];           // 该章节的段落列表
    segmentsMeta?: {                // 分段元数据
        start: number;
        end: number;
        charCount: number;
        status?: 'not_sent' | 'annotating' | 'done';
    }[];
    currentSegmentIndex?: number;    // 当前阅读到第几段
}
...
export interface StudyCourse {
    ...
    preference?: string;
    // 问答历史
    qaLogs?: { question: string; answer: string; timestamp: number }[];
}
